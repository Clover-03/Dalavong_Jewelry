...
CREATE TABLE `tb_product` (
    ...
    `status` ENUM('AVAILABLE', 'SOLD', 'REPURCHASED', 'DAMAGED', 'EXCHANGED') NOT NULL DEFAULT 'AVAILABLE',
...
CREATE TABLE `tb_repurchase` (
    ...
    `Re_Reason` TEXT NULL,
    `Damage_Cost` DECIMAL(10, 2) NULL DEFAULT 0.00,
    `Loose_Gold_Cost` DECIMAL(10, 2) NULL DEFAULT 0.00,
    `Net_Repurchase_Price` DECIMAL(10, 2) NULL DEFAULT 0.00,
...
CREATE TABLE `tb_exchange` (
    ...
    UNIQUE INDEX `tb_exchange_Old_Pd_ID_key`(`Old_Pd_ID`),
    PRIMARY KEY (`Exch_ID`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
